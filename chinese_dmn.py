...
def ATIS_data_load(path,encoding = "utf-8"):
    try:
...
train_data = ATIS_data_load(r'data-master/cross validation/cross_validation_train0.csv',encoding = "utf-8")
...
model = DMN(len(word2index), HIDDEN_SIZE, 22)
model.init_weight()
if USE_CUDA:
    model = model.cuda()
...
for epoch in range(EPOCH):
    ...
    for i,batch in enumerate(getBatch(BATCH_SIZE, train_data)):
        ...
        if i % 100 == 0:
            print("[%d/%d] mean_loss : %0.15f" %(epoch, EPOCH, np.mean(losses)))
...
test_data = ATIS_data_load(r'data-master/cross validation/cross_validation_train0.csv',encoding="utf-8")
...
print("")
print("Answer : ",(answer.data.tolist()[0]))
print("Prediction : ",(pred.max(1)[1].data.tolist()[0]))
