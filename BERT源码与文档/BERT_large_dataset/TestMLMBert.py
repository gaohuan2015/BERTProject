...
# This file is used to test the MLM-bert model
# --------------------------------****************************-----------------------------------------------------
import torch
...
if __name__ == "__main__":
    ...
    labels = create_examples(read_csv('DataSet/train0628.csv'))

    modelpath = 'PretrianedModel/MLM-bert-chinese'
    val_set = SSTDataset(tokerpath=modelpath, filename=test_data_path, maxlen=40)

    net = SentimentClassifier(modelpath=modelpath, shape=768, freeze_bert=False, num_labels=len(labels))
    net = net.to(device)

    model_save_pth = 'Models/MLM-bert-chinese_873.dat'
