...
# This file is used to test the time cost of our joint model in "CPU"
# --------------------------------****************************-----------------------------------------------------
import torch
...
def evaluate(net1,net2, net3, val_set):
    ...
    corre = 0
    with torch.no_grad():
        for it, i in enumerate(val_set):
                seq, attn_masks, labels = i[1].to(device), i[2].to(device), i[3].to(device)
...
if __name__ == "__main__":
    ...
    val_loader = DataLoader(val_set, batch_size=args.eval_batch_size, num_workers=1)
    val_set = []
    for i in val_loader:
        val_set.append(i)
